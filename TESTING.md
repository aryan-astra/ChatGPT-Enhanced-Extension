...
4. Select `x:\Personal\modus` folder
...
2. Look for messages starting with `[Modus]`
3. You should see:
   - `[Modus] Content script loaded at ...`
   - `[Modus] Settings loaded: {...}`
   - `[Modus] Checkboxes injected ... (bulkActions)`
   - `[Modus] Re-check: X checkboxes, icon-grid: ...`
...
**If not working:**
1. Check console for:
   - `[Modus] Compact sidebar initialized`
...
2. **Go to Console tab**
3. Look for `[Modus]` prefixed messages
4. Common messages:
   - `[Modus] Content script loaded` - Script is injected
   - `[Modus] Settings loaded` - Settings were read
   - `[Modus] Checkboxes injected N` - Checkboxes created
   - `[Modus] Compact sidebar initialized` - Icons setup
   - `[Modus] No checkboxes found, forcing re-injection` - Feature is retrying
...
**Check:**
1. Are you seeing `[Modus] Checkboxes injected` in console?
...
3. In DevTools, look for:
   - Class `modus-bulk-item` on the link
   - An `<input type="checkbox" class="modus-cb">` element inside
4. If missing, features aren't initializing properly

### Compact sidebar icons not appearing

**Check:**
1. Are you seeing `[Modus] Compact sidebar initialized` in console?
...
**Check:**
1. Do you see `[Modus] Content script loaded` in console?
